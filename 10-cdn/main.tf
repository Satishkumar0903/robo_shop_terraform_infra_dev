resource "aws_cloudfront_distribution" "devops-practice" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.devops-practice.domain_name
        zone_id = aws_cloudfront_distribution.devops-practice.hosted_zone_id
      }
    }
  ]
}